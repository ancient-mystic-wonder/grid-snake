...
public class ScoreDialog extends Table {
	...
	public ScoreDialog(Grid g, String deathType)
	{
		...
		int minutesMultiplier = (int)timer/30;
		
		String secondsString = String.format("%02d", seconds);
		
		String timeText = Integer.toString(minutes)+":"+secondsString;
		scoreDialog.add(new Label("Time Bonus: " + timeText + " - " + Integer.toString(minutesMultiplier+1) + "x",labelStyle)).expand();
		scoreDialog.row();
		
		
		// length bonus
		scoreDialog.add(new Label("Length Bonus: " + grid.bestLength + " - " + ((grid.bestLength-3)/5+1) + "x",labelStyle)).expand();
		scoreDialog.row();
